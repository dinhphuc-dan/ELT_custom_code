...
class Google_IAP_to_Bigquery():
    def __init__(self, store_name, slack_channel, google_play_bucket, service_account, project_id, dataset_id, logger):
    ...
    def check_IAP_from_console_then_load_to_bigquery(self):
        ...
        list_files_in_bucket: list = [x.name for x in bucket.list_blobs(prefix='sales/')] # get list file in storage
        self.logger.info(f'{self.store_name} Number file in Bucket: {len(list_files_in_bucket)}')
        ...
        list_date_in_google_console_files: list = sorted({re.search(r'\d{6}', file).group() for file in list_files_in_bucket}, reverse=True)
        ...
        list_tables_in_dataset: list = [x.table_id for x in list_tables_in_dataset if re.search(r"salesreport_", x.table_id)]

        self.logger.info(f'{self.store_name} Number table in Bigquery: {len(list_tables_in_dataset)}')
        if len(list_tables_in_dataset) > 0: # comparing between Console and Bigquery
            list_date_in_bigquery_table = sorted({re.search(r'\d{6}', table).group() for table in list_tables_in_dataset}, reverse=True)
            list_date_not_in_bigquery : list = [x for x in list_date_in_google_console_files if x not in list_date_in_bigquery_table]
        ...
        if len(list_tables_in_dataset) == 0: 
            for file in list_files_in_bucket: 
                response_in_bytes = BytesIO(bucket.blob(file).download_as_bytes()) # download file from Console as bytes then put it into BytesIO to make an in-memory file
                ...
                for name in zip_file.namelist(): # get each file in zip file
                    ...
                    # Make an API request
                    load_job = bqclient.load_table_from_file(file_obj= csv_file, destination=bq_table_id, job_config=job_config)
                    load_job.result()  # Waits for the job to complete
        else:
            list_date_not_in_bigquery.append(list_date_in_google_console_files[0]) # append lastest-month file
            list_date_not_in_bigquery = set(list_date_not_in_bigquery) # turn to set to make list unique
            ...
            for date in list_date_not_in_bigquery:
                ...
                response_in_bytes = BytesIO(bucket.blob(file_name).download_as_bytes()) # download file from Console as bytes then put it into BytesIO to make an in-memory file
                zip_file = ZipFile(response_in_bytes, "r") # unzip file
                for name in zip_file.namelist(): # get each file in zip file
                    csv_file = BytesIO(zip_file.read(name)) # read file from zip file as bytes then put it into BytesIO to make an in-memory file
                    bq_table_id = dataset + '.' + re.match(r'[a-z0-9_]+', name).group() # set bigquery table id
...
def IAP_main_flow(date=datetime.datetime.now()):
    ...
    for store in list_store:
        ...
        list_instances.append(Google_IAP_to_Bigquery(
